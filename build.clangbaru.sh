...
LOCAL_DIR=/home/JFla/
ZIPNAME="JFla-Karamel -$(TZ=Asia/Jakarta date +"%Y%m%d-%H%M").zip"
ZIPNAME_KSU="JFla-Karamel-KSU-$(TZ=Asia/Jakarta date +"%Y%m%d-%H%M").zip"
...
sed -i 's/CONFIG_KSU=n/CONFIG_KSU=y/g' arch/arm64/configs/onclite-perf_defconfig
sed -i 's/CONFIG_LOCALVERSION="-"JFla-Karamel/CONFIG_LOCALVERSION="-JFla-Karamel-KSU"/g' arch/arm64/configs/onclite-perf_defconfig
sed -i 's/CONFIG_MSM_CAMERA_BOOTCLOCK_TIMESTAMP=y/#CONFIG_MSM_CAMERA_BOOTCLOCK_TIMESTAMP=is not set/g' arch/arm64/configs/onclite-perf_defconfig
else
...
git restore arch/arm64/configs/onclite-perf_defconfig
if [ -d "$AK3_DIR" ]; then
cp -r $AK3_DIR AnyKernel3
elif ! git clone -q https://github.com/PapiLer/AnyKernel3; then
